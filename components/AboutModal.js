      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            <button
              onClick={onClose}
              className="absolute top-6 right-6 w-10 h-10 bg-neutral-medium/20 hover:bg-neutral-medium/40 rounded-full flex items-center justify-center transition-colors duration-200 z-10"
            >
              <i className="fas fa-times text-neutral-dark"></i>
            </button>
            ...
            <div className="p-8 md:p-12">
              ...
              >
                <div className="w-20 h-20 bg-gradient-blue rounded-full flex items-center justify-center mx-auto mb-6">
                  <i className="fas fa-rocket text-3xl text-white"></i>
                </div>
                <h2 className="text-3xl md:text-4xl font-bold text-neutral-dark mb-4">
                  Ihr Partner für professionelle AI-Agent-Entwicklung
                </h2>
                <p className="text-lg text-neutral-text max-w-3xl mx-auto leading-relaxed">
              ...
              >
                <div className="text-center mb-8">
                  <h3 className="text-2xl font-bold text-neutral-dark mb-2">Unsere Expertise</h3>
                  <p className="text-neutral-text">Maßgeschneiderte Lösungen für jeden Anwendungsfall</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {expertise.map((item, index) => (
                    <motion.div
                      ...
                      className="bg-neutral-light/50 rounded-xl p-6 text-center"
                    >
              ...
              >
                <div className="text-center mb-8">
                  <h3 className="text-2xl font-bold text-neutral-dark mb-2">Für wen wir arbeiten</h3>
                  <p className="text-neutral-text">Unsere AI-Agenten unterstützen Unternehmen jeder Größe</p>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  {industries.map((industry, index) => (
                    <motion.div
                      ...
                      className="bg-white border-2 border-neutral-medium/30 rounded-xl p-6 text-center hover:border-primary-blue/30 transition-colors duration-300"
                    >
                      <i className={`${industry.icon} text-3xl gradient-text mb-3 block`}></i>
                      <h4 className="font-semibold text-neutral-dark text-sm">{industry.title}</h4>
                    </motion.div>
                  ))}
                </div>
              </motion.div>
              ...
              >
                <div className="text-center mb-8">
                  <h3 className="text-2xl font-bold text-neutral-dark mb-2">Warum DentalBotPro?</h3>
                  <p className="text-neutral-text">Die Vorteile unserer Lösungen</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {benefits.map((benefit, index) => (
                    <motion.div
                      ...
                      className="bg-neutral-light/30 rounded-xl p-6 flex items-start space-x-4"
                    >
                      <div className="w-12 h-12 bg-gradient-blue rounded-lg flex items-center justify-center flex-shrink-0">
                        <i className={`${benefit.icon} text-lg text-white`}></i>
                      </div>
                      <div>
                        <h4 className="font-semibold text-neutral-dark mb-2">{benefit.title}</h4>
                        <p className="text-sm text-neutral-text">{benefit.description}</p>
                      </div>
                    </motion.div>
                  ))}
                </div>
              ...
              >
                <div className="text-center mb-8">
                  <h3 className="text-2xl font-bold text-neutral-dark mb-2">Unsere Highlights</h3>
                  <p className="text-neutral-text">Was uns besonders macht</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {highlights.map((highlight, index) => (
                    <motion.div
                      ...
                      className="bg-gradient-light rounded-xl p-6 text-center"
                    >
                      <i className={`${highlight.icon} text-3xl gradient-text mb-4 block`}></i>
                      <h4 className="font-semibold text-neutral-dark mb-2">{highlight.title}</h4>
                      <p className="text-sm text-neutral-text">{highlight.description}</p>
                    </motion.div>
                  ))}
                </div>
              </motion.div>
              ...
              <motion.div
                ...
                className="text-center bg-gradient-light rounded-2xl p-8"
              >
                <h3 className="text-2xl font-bold text-neutral-dark mb-4">
                  Bereit für Ihren AI-Agenten?
                </h3>
                <p className="text-neutral-text mb-6 max-w-2xl mx-auto">
